...
class WC_Meta_Box_Braintree_Subscription_Data {
	...
	public static function enqueue_scripts() {
		...
		if ( $screen_id === 'product' ) {
			...
			wp_enqueue_script( 'wc-braintree-subscription-product-data', $js_path . 'admin/meta-boxes-product.js', array( 'jquery', 'jquery-blockui' ), braintree()->version, true );
			wp_localize_script(
				'wc-braintree-subscription-product-data',
				'wc_braintree_meta_box_product_params',
				array(
					'_wpnonce'     => wp_create_nonce( 'wp_rest' ),
					'url'          => array( 'plans' => braintree()->rest_api->plans->rest_url() . 'fetch' ),
					'plans'        => array(
