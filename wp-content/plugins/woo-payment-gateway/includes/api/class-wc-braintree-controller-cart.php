...
class WC_Braintree_Controller_Cart extends WC_Braintree_Controller_Frontend {
	...
	public function add_to_cart( $request ) {
		...
		WC()->cart->remove_cart_item( WC()->cart->generate_cart_id( $product_id, $variation_id ) );
		...
		try {
			...
			return rest_ensure_response(
				apply_filters(
					...
					array(
						'data' => $gateway->update_shipping_method_response( array( 'chosen_shipping_methods' => $chosen_shipping_methods ) ),
					),
					$payment_method,
					$request
				)
