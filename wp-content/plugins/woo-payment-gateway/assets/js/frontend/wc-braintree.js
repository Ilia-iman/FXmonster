(function ($) {
        ...
        wc_braintree.PayPal.prototype.create_button = function () {
            return new Promise(function (resolve, reject) {
                ...
                if ($buttonContainer.length) {
                    $buttonContainer.find('.paypal-buttons').remove();
                    this.get_funding().sort(function (a, b) {
                        ...
                        var button = paypal.Buttons(this.render_options(source));
                        if (button.isEligible()) {
                            button.render($buttonContainer[0]);
        ...
        wc_braintree.PayPal.prototype.create_bnpl_msg = function (section, container) {
            if (this.is_bnpl_msg_enabled(section) && $(container).length) {
                $(container).parent().find('.wc-braintree-pay-later-msg').remove();
        ...
        wc_braintree.GooglePay.prototype.update_addresses = function (paymentData) {
            if (paymentData.paymentMethodData.info.billingAddress) {
                ...
                // if address is valid, don't overwrite with GPay billing address. But use phone
                if (this.is_checkout_page() && this.is_valid_address(this.billing_checkout_address, 'billing', ['phoneNumber'])) {
